...
impl TronMcpServer {
    ...
    /// Send a command to the game server and get response
...
impl TronMcpServer {
    #[tool(
        description = "Join the next available Tron light-cycle game. You will be matched with other players. Once the game starts, use 'look' to see the grid and 'steer' to change direction. Your light-cycle moves forward automatically every tick."
    )]
    fn join_game(
        &self,
        Parameters(params): Parameters<JoinGameParams>,
        ...
        *self.player_name.lock().map_err(|e| {
            McpError::internal_error(format!("Lock error: {}", e), None)
        })? = Some(name.clone());
    ...
    #[tool(
        description = "Look at the game grid around your light-cycle. Returns a text map showing your position (@), your trail (|), other players and their trails (1-9), walls (#), obstructions (X), and empty space (.). Use this to plan your moves and avoid collisions!"
    )]
    fn look(&self) -> Result<CallToolResult, McpError> {
        let name = self.player_name.lock().map_err(|e| {
            McpError::internal_error(format!("Lock error: {}", e), None)
        })?;
        let name = name.as_ref().ok_or_else(|| {
            McpError::invalid_params("Use join_game first.", None)
        })?;
    ...
    fn steer(
        ...
        if !["left", "right", "straight"].contains(&dir.as_str()) {
            return Ok(CallToolResult::error(vec![Content::text(
                "Direction must be 'left', 'right', or 'straight'.",
            )]));
        }
    ...
    #[tool(
        description = "Get the current game status: whether the game is waiting, running, or finished, your score, the winner, and the leaderboard standings. Use this after the game ends to see results. If you won, use join_game again to play the next level!"
    )]
...
impl ServerHandler for TronMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(
                "Tron Light-Cycle MCP Game! You control a light-cycle on a grid. \
